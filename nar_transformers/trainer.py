...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        tr_losses = {
            "ctc": torch.tensor(0.0).to(args.device),
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        tr_loss_step, tr_losses_step = self.training_step(model, inputs, beta=beta_list[self.state.global_step])
                else:
                    tr_loss_step, tr_losses_step = self.training_step(model, inputs, beta=beta_list[self.state.global_step])
    ...
    def _maybe_log_save_evaluate(self, tr_loss, tr_losses, beta, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log:
            ...
            logs["loss_CTC"] = round(tr_losses_scalar["ctc"] / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["loss_KL"] = round(tr_losses_scalar["kl"] / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["beta"] = beta
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], beta=0.0) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            ...
            loss = outputs.loss + beta * outputs.loss_kl
            losses = {
                "ctc": outputs.loss.detach(),
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits, labels = self._prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
