...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        # For beta anealing
        beta_list = self.frange_cycle_zero_linear(max_steps, start=0.0, stop=1.0, n_cycle=1*num_train_epochs,
    ...
    def _prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    ...
                    losses = {
                        #'rec': outputs.loss.mean().detach(),
                        #'length': outputs.loss_length.mean().detach(),
                        #'kl': outputs.loss_kl.mean().detach(),
                        'rec': loss,
                        'length': loss,
                    }
        ...
        if prediction_loss_only:
            return (loss, losses, None, None)

        logits = nested_detach(logits)
        if len(logits) == 1:
            logits = logits[0]

        return (loss, losses, logits, labels)
