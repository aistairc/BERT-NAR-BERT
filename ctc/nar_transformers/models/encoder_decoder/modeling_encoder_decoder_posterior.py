...
class EncoderDecoderPosteriorModel(PreTrainedModel):
    r"""
    ...
    def __init__(
        ...
        self.linear_p = nn.Linear(config.encoder.hidden_size, 2 * latent_size, bias=False)
        self.linear_q = nn.Linear(config.encoder.hidden_size, 2 * latent_size, bias=False)
        self.linear_h = nn.Linear(latent_size, config.decoder.hidden_size, bias=False)
        #self.linear_z = nn.Linear(config.encoder.hidden_size, latent_size, bias=False)

        from ..bert.modeling_bert import BertModel
        self.x_encoder = BertModel(config.encoder)
        self.y_encoder = BertModel(config.encoder)
    ...
    def forward(
        ...
        **kwargs,
        ...
        prior_states = encoder_outputs[0]
        x_mask = attention_mask

        # Approximate Posterior
        if self.training:
            y_encoder_outputs = self.y_encoder(
            ...
            y_encoder_hidden_states = y_encoder_outputs[0]
            x_encoder_outputs = self.x_encoder(
                input_ids=input_ids,
                attention_mask=attention_mask,
                encoder_hidden_states=y_encoder_hidden_states,
            ...
            posterior_states = x_encoder_outputs[0]

            mu_p, var_p = self.linear_p(prior_states).chunk(2, -1)
            var_p = nn.functional.softplus(var_p)
            mu_q, var_q = self.linear_q(posterior_states).chunk(2, -1)
            var_q = nn.functional.softplus(var_q)
            loss_kl = (var_p / (var_q + 1e-10) + 1e-10).log() + (
                        (var_q.pow(2) + (mu_q - mu_p).pow(2)) / (2 * (var_p).pow(2))
                    ) - 0.5
            #loss_kl = ((loss_kl.mean(-1) * x_mask).sum(1) / x_mask.sum(1)).mean()
            loss_kl = ((loss_kl.sum(-1) * x_mask).sum(1) / x_mask.sum(1)).mean()

            eps = torch.randn_like(var_q)
            z = mu_q + eps * var_q

        else:
            mu_p, logvar_p = self.linear_p(prior_states).chunk(2, -1)
            z = mu_p
        ...
        decoder_outputs = self.decoder(
            ...
            encoder_hidden_states=prior_states,
