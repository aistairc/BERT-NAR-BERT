    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                elif len(tmp_output) == 3:
                    tmp_eval_loss, logits, hidden = tmp_output
        ...
        out_file_name = "eval_results" + "_lr_" + str(args.learning_rate) + ".txt"
        output_eval_file = os.path.join(eval_output_dir, out_file_name)
        with open(output_eval_file, "a") as writer:
            logger.info("***** Eval results {} *****".format(prefix))
            if args.use_freeze:
                writer.write("***** Running evaluation checkpoint on {} with Feature-based Approach: {} *****\n".format(prefix, args.use_freeze))
            else:
                writer.write("##### Model Path = {}\n".format(args.model_name_or_path))
                writer.write("***** Running Evaluation Checkpoint = {}\n".format(prefix))
            writer.write("Learning_Rate = %s\n" % (args.learning_rate))
            writer.write("Learning_Seed = %s\n" % (args.seed))
...
def main():
    ...
    parser.add_argument("--log_path", default=None, type=str, required=True, help="Path to the logging file.",)
