...
class EncoderDecoderModel(PreTrainedModel):
    ...
    print("Use Model: ", base_model_prefix)
    ...
    def forward(
        ...
        **kwargs
        ...
        #next_token_logits = decoder_outputs.logits[0][0, -1, :] / 1.0
        #tokens_ids = torch.multinomial(F.softmax(next_token_logits, dim=-1), num_samples=1).flatten()
        #tokens_ids = torch.multinomial(F.softmax(decoder_outputs.logits[0][0], dim=-1), num_samples=1).flatten()
        #tokens_ids = torch.multinomial(decoder_outputs.logits[0][0], num_samples=1).flatten()
